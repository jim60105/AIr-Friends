...
Deno.test("SandboxManager - filterEnv: true filters out non-allowed env vars", () => {
  ...
  const opts = sandbox.buildSpawnOptions("copilot", "copilot", ["--agent"], baseEnv, "/tmp");
...
Deno.test("SandboxManager - copilot agent allows GITHUB_TOKEN but not GEMINI_API_KEY", () => {
  const sandbox = new SandboxManager(createSandboxConfig());
  const baseEnv = {
    PATH: "/usr/bin",
    GITHUB_TOKEN: "ghp_xxx",
...
Deno.test("SandboxManager - gemini agent allows GEMINI_API_KEY and GEMINI_SYSTEM_MD", () => {
  const sandbox = new SandboxManager(createSandboxConfig());
  const baseEnv = {
    PATH: "/usr/bin",
    GEMINI_API_KEY: "gm_xxx",
    GEMINI_SYSTEM_MD: "/app/prompts/system.md",
    GITHUB_TOKEN: "ghp_xxx",
  };
  const opts = sandbox.buildSpawnOptions("gemini", "gemini", [], baseEnv, "/tmp");
  assertEquals(opts.env["GEMINI_API_KEY"], "gm_xxx");
  assertEquals(opts.env["GEMINI_SYSTEM_MD"], "/app/prompts/system.md");
  assertEquals(opts.env["GITHUB_TOKEN"], undefined);
});

Deno.test("SandboxManager - opencode agent allows multiple API keys", () => {
  const sandbox = new SandboxManager(createSandboxConfig());
  const baseEnv = {
    PATH: "/usr/bin",
    GEMINI_API_KEY: "gm_xxx",
...
Deno.test("SandboxManager - networkIsolation on Linux wraps with unshare", () => {
  ...
  const opts = sandbox.buildSpawnOptions("copilot", "copilot", ["--acp"], {}, "/tmp");
  assertEquals(opts.command, "unshare");
  assertEquals(opts.args, ["--net", "copilot", "--acp"]);
});
...
Deno.test("SandboxManager - networkIsolation false does not wrap command", () => {
  ...
  const opts = sandbox.buildSpawnOptions("copilot", "copilot", ["--acp"], {}, "/tmp");
  assertEquals(opts.command, "copilot");
  assertEquals(opts.args, ["--acp"]);
});

Deno.test("SandboxManager - cwd is passed through", () => {
  const sandbox = new SandboxManager(createSandboxConfig({ filterEnv: false }));
  const opts = sandbox.buildSpawnOptions("copilot", "copilot", [], {}, "/my/workspace");

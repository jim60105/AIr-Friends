Feature: Channel Lurk Reply
  As a bot operator
  I want the bot to periodically check configured channels
  ...
  Background:
    ...
    And the channels list contains Discord channel entries

  Scenario: All conditions met - triggers agent reply
    Given the last message in a configured channel is from a non-bot user
  ...
  Scenario: Skip - last message from bot
    Given the last message in a configured channel is from the bot itself
    When the channel lurk interval elapses
  ...
  Scenario: Skip - no messages in channel
    Given a configured channel has no messages
    When the channel lurk interval elapses
  ...
  Scenario: No channels configured for channel lurk
    Given channel lurk is enabled but no Discord channels with channelLurk: true are in the channels list
    When the application starts

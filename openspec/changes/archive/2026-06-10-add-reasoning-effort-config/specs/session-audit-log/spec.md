...
- **WHEN** a message session is registered with workspace key `discord/123`, agent type `copilot`, model `claude-opus-4.6`, reasoning effort `high`, and YOLO mode disabled
- **THEN** a `session_start` entry SHALL be written with `sessionType: "message"`, `workspaceKey: "discord/123"`, `agentType: "copilot"`, `model: "claude-opus-4.6"`, `reasoningEffort: "high"`, `yolo: false`

...
**Reason**: The composition loaded all high-importance memories without a bound. Fixed memories are now selected by tier within token budgets.
**Migration**: Replaced by "Initial Context Composition with Fixed Budgets". Memories left out remain reachable through `memory-search`.

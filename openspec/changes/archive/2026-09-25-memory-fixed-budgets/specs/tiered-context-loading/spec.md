...
**Reason**: Unbounded core loading makes every prompt grow with the memory store. Core loading is now budgeted.
**Migration**: Replaced by "Core Tier Loaded Within Token Budget". Core memories left out by the budget remain reachable through Fast Recall and `memory-search`.

### Requirement: Working Tier Loads Recent N Entries
**Reason**: Injection count was tied to `workingTierLimit`, which also governs storage demotion. Injection now has its own count and token budget.
**Migration**: Replaced by "Working Tier Loads Newest Entries Within Budget". Configure `memory.recall.workingMaxItems` and `memory.recall.workingMaxTokens`.

### Requirement: Archive Tier Search-Only
**Reason**: Archive memories are now also reachable through automatic Fast Recall.
**Migration**: Replaced by "Archive Tier Retrieval-Only".

### Requirement: Channel Core and Working Memories Loaded in Channel Context
**Reason**: Channel memories now share the budgeted core and working selection with user memories.
**Migration**: Replaced by "Channel Memories Share Fixed Budgets".

...
- **`scoreMemories(population, query, hints, now, supersededIds?)`** computes `N`, `df` (for the query keys only) and `avgdl` from the given population in one pass, then scores every document with at least one matched key. The caller decides what the population is, which is how change 3 keeps private memories out of guild statistics. The optional `supersededIds` is the authoritative supersede set for the temporal bonus: the definition spans every enabled memory in scope, which can be wider than a population already narrowed by a `category` or `scope` filter. When it is omitted the set is inferred from the population.

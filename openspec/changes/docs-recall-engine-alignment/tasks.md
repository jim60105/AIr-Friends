...
- [ ] 1.4 Skill-list line (~line 346): rewrite "- `memory-search` — Search memory by keywords" as natural-language retrieval via the recall engine.
...
- [ ] 3.1 `docs/CONTAINER_TOOLS.md` (~line 26): drop "Used internally by the memory search skill" from the `ripgrep` table entry; purpose becomes agent-side file reading under the ACP permission gate.
- [ ] 3.2 AGENTS.md container-binary line (~1321) "rg - ripgrep 15.1.0 for memory search": fix the same way as task 1.3. Leave the `searchLimit`/`maxChars` example lines (~391-392) untouched.

## 4. Verification

- [ ] 4.1 Grep-verify no stale wording remains: `grep -rniE "always loaded|automatically loaded|full-text search|search keywords|for memory search|by keywords|used internally by the memory" docs/DESIGN.md docs/SKILLS_IMPLEMENTATION.md docs/CONTAINER_TOOLS.md AGENTS.md` returns no matches.

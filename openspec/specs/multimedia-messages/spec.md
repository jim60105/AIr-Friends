...
Images exceeding 20 MB SHALL NOT be downloaded; they SHALL be described by URL only. Image downloads SHALL have a 10-second timeout. Download failures (timeout, network error, HTTP error) SHALL be non-fatal — the system SHALL fall back to URL-only text description without throwing errors.
